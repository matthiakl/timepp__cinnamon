 ...
 *
 * pattern, txt -> score (or null)
 *
 * @pattern: string (Pattern to look for.)
 * @txt:     string (Text in wihch to look for pattern.)
 * @pattern: int    (Match score. A higher score means a better match.)
 *
 ...
 * a shorter version. If a single pattern letter is missing from the text, we
 * return null.
 *
 *     a  b  c d e  abcdef
 *     ----------------->|
 ...
 * found, whether the letters appear at word begginings, and the number of gaps.
 */
function fuzzy_search_v1 (pattern, txt) {
    ...
    let gaps            = 0;
    let consecutives    = 0;
    let word_begginings = 0;
    let last_match_idx  = 0;

    if (pattern[0] === txt[0]) word_begginings++;

    while ( (i--) > start_pos) {
        if (txt[i] === pattern[patt_pos]) {
            if (i+1 === last_match_idx) consecutives++;
            if (/\W/.test(txt[i-1])) word_begginings++;
            last_match_idx = i;
            patt_pos--;
        }
        else gaps++;
    ...
    return (consecutives * 4) + (word_begginings * 3) - gaps - start_pos;
}
...
/*
 * pattern, txt ,k -> levenshtein distance
 *
 * A lower returned distance means the match is better. 0 means perfect match.
 *
 * @pattern: string (Pattern to look for.)
 * @txt:     string (Text in wihch to look for pattern.)
 ...
 * (levenshtein disctances) should be returned as the final score. The user
